    ...
    return expr.cases({
        ...
        return recurse(list).cases({
          ...
          squishList: function(list) {
            if (list._tag === 'list' && !body.containsInNonTailPosition(Core.variable(accum))) {
              DEBUG.log("> deforesting fold over squish in " + (this.inspect()));
      ...
      squishList: function(list) {
        ...
        for (_i = 0, _len = _ref10.length; _i < _len; _i++) {
          ...
          e = recurse(e);
          if (e.alwaysFails()) {
            DEBUG.log("> dropping failed " + e + " from squish list");
            continue;
          }
          nonFailures.push(e);
          if (!e.alwaysSucceeds()) {
        ...
        if (allSucceed) {
          DEBUG.log("> unsquishing successful list " + (Core.list(nonFailures).inspect()));
